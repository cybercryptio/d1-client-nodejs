...
export abstract class D1BaseClient {
    ...
    constructor(serverUrl: string, creds: ICredentials, certPath: string | undefined = undefined) {
        ...
        this.authn = new d1.authn.Authn((method, requestData, callback) => {
            this.client.makeUnaryRequest(`/d1.authn.Authn/${method.name}`, arg => arg as Buffer, arg => arg as Buffer, requestData, callback);
        });
        this.authz = new d1.authz.Authz((method, requestData, callback) => {
            this.client.makeUnaryRequest(`/d1.authz.Authz/${method.name}`, arg => arg as Buffer, arg => arg as Buffer, requestData, callback);
        });
        this.version = new d1.version.Version((method, requestData, callback) => {
            this.client.makeUnaryRequest(`/d1.version.Version/${method.name}`, arg => arg as Buffer, arg => arg as Buffer, requestData, callback);
        });
        this.index = new d1.index.Index((method, requestData, callback) => {
            this.client.makeUnaryRequest(`/d1.index.Index/${method.name}`, arg => arg as Buffer, arg => arg as Buffer, requestData, callback);
        });
    }
}